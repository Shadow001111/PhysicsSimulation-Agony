...
namespace PS_AGONY::FastCosSin
{
	...
	)
	{
		...
		// Math contants.
		constexpr Real coeff1 = 3.0 / Constants::PI;
		constexpr Real coeff2 = 4.0 / (Constants::PI * Constants::PI * Constants::PI);

		const RealSimd coeff1V(coeff1);
		...
		for (; i + LANES <= size; i += LANES)
		{
			...
			RealSimd sinMag = a * RealSimd::neg_mul_add(a * a, coeff2V, coeff1V);
		...
		for (; i < size; i++)
		{
			...
			int q = (int)(x * Real(Constants::INV_HALF_PI));
			Real u = x - q * Real(Constants::HALF_PI);
			...
			Real sin = sin_sign * a * (coeff1 - (a * a * coeff2));
	...
	)
	{
		...
		// Math contants.
		constexpr Real coeff2 = 4.0 * (3.0 - Constants::PI) / (Constants::PI * Constants::PI);
		constexpr Real coeff3 = 4.0 * (Constants::PI - 4.0) / (Constants::PI * Constants::PI * Constants::PI);
		...
		for (; i + LANES <= size; i += LANES)
		{
			...
			RealSimd a = RealSimd::mul_add(
				mirror,
				RealSimd::neg_mul_add(RealSimd(2.0f), u, halfPiV),
				u
			);
			...
			RealSimd sinMag = a * RealSimd::mul_add(RealSimd::mul_add(a, coeff3V, coeff2V), a, RealSimd(1));
			RealSimd cosMag = RealSimd::sqrt(RealSimd::neg_mul_add(sinMag, sinMag, RealSimd(1)));
		...
		for (; i < size; i++)
		{
			...
			Real sin = sin_sign * a * (Real(1) + a * (coeff2 + a * coeff3));
			Real cos = cos_sign * std::sqrt(Real(1) - sin * sin);
