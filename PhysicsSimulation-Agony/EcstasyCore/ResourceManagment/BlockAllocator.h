...
namespace Ectasy::ResourceManagment
{
	...
	private:
		using FreeByOffset = std::map<TIndex, TIndex>;
		using FreeBySize = std::multimap<TIndex, TIndex>;
	...
	public:
		explicit RangeAllocator(TIndex capacity = 0) :
			capacity(capacity)
		{
			if (capacity > 0)
			{
