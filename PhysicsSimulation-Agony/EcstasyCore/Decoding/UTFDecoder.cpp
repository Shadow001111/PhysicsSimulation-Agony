...
namespace Ecstasy::Decoding
{
    ...
    {
        if (index >= length) return INVALID_UTF_CODEPOINT;
        ...
        if ((c & 0xE0) == 0xC0)
        {
            ...
            {
                index = length;
                return INVALID_UTF_CODEPOINT;
            }
            uint32_t cp = ((c & 0x1F) << 6) | (static_cast<uint8_t>(buffer[index + 1]) & 0x3F);
            index += 2;
            return cp < 0x80 ? INVALID_UTF_CODEPOINT : cp;
        }
        ...
        if ((c & 0xF0) == 0xE0)
        {
            ...
            index += 3;
            return cp < 0x800 ? INVALID_UTF_CODEPOINT : cp;
        }
        ...
        if ((c & 0xF8) == 0xF0)
        {
            ...
            index += 4;
            return (cp < 0x10000 || cp > 0x10FFFF) ? INVALID_UTF_CODEPOINT : cp;
        }
