...
>
void radixSort(
    T* CORE_RESTRICT arrayToBeSorted,
    T* CORE_RESTRICT tempArray,
    SizeType count)
{
...
void radixSortIndices(
    IndexType* CORE_RESTRICT indexArrayToBeSorted,
    IndexType* CORE_RESTRICT tempIndexArray,
    const KeyType* CORE_RESTRICT keyArray,
    SizeType count)
{
    static_assert(RADIX_BITS == 8 || RADIX_BITS == 16 || RADIX_BITS == 32 || RADIX_BITS == 64,
        "RADIX_BITS must be 8, 16, 32, or 64");
